print("--- CARGANDO PROTOCOLO SR ---")
-- 1. Definición del protocolo
local mi_proto = Proto("SR", "selective_repeat")
...
function mi_proto.dissector(buf, pkt, tree)
    ...
    -- Tu formato tiene 6 campos fijos antes de la data: 
...
function mi_proto_heuristic(buf, pkt, tree)
    ...
    -- Extraemos los primeros caracteres para ver si es "SR"
    local signature = "selective_repeat"
    ...
    print("DEBUG - Wireshark leyó: [" .. prefix .. "]")
    ...
    -- Si no empieza con "SR", devolvemos false para que otro script intente leerlo
    return false
end


-- 4. Registro del puerto 
--local udp_table = DissectorTable.get("udp.port")
--udp_table:add(8081, mi_proto)
mi_proto:register_heuristic("udp", mi_proto_heuristic)
print("--- REGISTRO HEURÍSTICO OK ---")
